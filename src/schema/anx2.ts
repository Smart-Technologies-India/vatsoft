...
import { InferInput, enum_, maxLength, minLength, object, string, pipe } from "valibot"
...
const Anx2Schema = object({
  ...
  areaName: pipe(string(), minLength(1, "Area Name is required.")),
  village: pipe(string(), minLength(1, "Village is required.")),
  pinCode: pipe(string(), minLength(1, "Pincode is required.")),
  dateOfExtablishment: pipe(string(), minLength(1, "Date of Extablishment is required.") ,),
