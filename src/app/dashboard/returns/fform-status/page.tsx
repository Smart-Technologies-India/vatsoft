...
import { fform, dvat04 } from "@prisma/client";
...
const TrackAppliation = () => {
  ...
  const init = async () => {
    ...
    if (dvat_response.data && dvat_response.status) {
      ...
      const fform_data = await GetUserFform({
        dvatid: dvat_response.data.id,
        take: 10,
        skip: 0,
      });

      if (fform_data.status && fform_data.data.result) {
        setFformData(fform_data.data.result);
        setPaginatin({
          skip: fform_data.data.skip,
          take: fform_data.data.take,
          total: fform_data.data.total,
        });
      }
    }
  ...
  const tinsearch = async () => {
    ...
    const search_response = await GetUserFform({
      dvatid: dvatdata.id,
    ...
    if (search_response.status && search_response.data.result) {
      setFformData(search_response.data.result);
      setPaginatin({
            ...
            <div className="flex flex-col lg:flex-row gap-3 items-start lg:items-center">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  View and track all your F-Form declarations and submissions
                </p>
              </div>
          ...
          {fformData.length == 0 && (
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4">
              <Alert
                type="info"
                showIcon
                message="No F-Forms Found"
                description="There are no F-Form declarations available."
              />
            </div>
          )}
              ...
              <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                <div className="overflow-x-auto">
                  <Table>
                    ...
                    <TableBody>
                      {fformData.map((val: fform, index: number) => (
                        <TableRow
                          key={index}
                        ...
                        >
                          <TableCell className="border border-gray-200 text-center p-3 text-xs">
                            <Link
                              href={`/dashboard/fform/${encryptURLData(
                                val.id.toString(),
                              )}`}
