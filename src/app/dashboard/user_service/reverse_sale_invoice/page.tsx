...
import GetReverseInvoiceSale, {
  ReverseInvoiceSaleRow,
} from "@/action/stock/getreverseinvoicesale";
import ReverseSaleAccept from "@/action/stock/reversesaleaccept";
import {
  ColumnDef,
  ColumnFiltersState,
  FilterFn,
...
interface GroupedSaleRow {
  groupKey: string;
  invoice_date: Date;
  invoice_number: string;
  tin_number: string;
  trade_name: string;
  records: ReverseInvoiceSaleRow[];
...
const ReverseSaleInvoicePage = () => {
  ...
  const [rows, setRows] = useState<Array<ReverseInvoiceSaleRow>>([]);
  ...
  const [selectedGroup, setSelectedGroup] = useState<GroupedSaleRow | null>(
    null,
  );
  ...
  const loadRows = useCallback(async () => {
    const reverseSaleResponse = await GetReverseInvoiceSale();
    if (reverseSaleResponse.status && reverseSaleResponse.data) {
      setRows(reverseSaleResponse.data);
    ...
    setRows([]);
    setPagination((prev) => ({ ...prev, pageIndex: 0 }));
    if (reverseSaleResponse.message) {
      toast.error(reverseSaleResponse.message);
    }
  }, []);

  const onReverse = useCallback(
    async (row: ReverseInvoiceSaleRow) => {
      if (reversingIds.includes(row.id)) return;

      setReversingIds((prev) => [...prev, row.id]);
      const response = await ReverseSaleAccept({
        saleId: row.id,
        updatedById: userid,
      });
  ...
  const onReverseAll = useCallback(
    async (group: GroupedSaleRow) => {
      ...
      if (successCount > 0) {
        toast.success(`${successCount} sale(s) reversed successfully.`);
      }
      if (failedCount > 0) {
        toast.error(`${failedCount} sale(s) could not be reversed.`);
      }
  ...
  const urnFilterFn: FilterFn<ReverseInvoiceSaleRow> = (
    row,
    _,
    filterValue,
  ) => {
  ...
  const groupedRows = useMemo(() => {
    const groups: Record<string, GroupedSaleRow> = {};
  ...
  const groupColumns = useMemo<ColumnDef<GroupedSaleRow>[]>(
    () => [
      {
        id: "sr_no",
        header: "Sr. No.",
  ...
  const columns = useMemo<ColumnDef<ReverseInvoiceSaleRow>[]>(
    () => [
      {
        id: "sr_no",
        header: "Sr. No.",
      ...
      <div className="max-w-7xl mx-auto">
        <div className="bg-white border border-gray-200 p-3 rounded-lg shadow-sm mb-3">
          ...
          <p className="text-xs text-gray-600 mt-1">
            Showing sale entries where buyer TIN exists in TIN master but has no DVAT registration.
          </p>
        ...
        <div className="bg-white rounded shadow-sm border p-3">
          <div className="mb-3 grid grid-cols-1 gap-2 md:grid-cols-4">
            ...
            <Select
              allowClear
              placeholder="Filter URN"
              options={[
                { label: "With URN", value: "with_urn" },
                { label: "Without URN", value: "without_urn" },
              ]}
              onChange={(value) => {
                table.getColumn("urn_number")?.setFilterValue(value || undefined);
                setPagination((prev) => ({ ...prev, pageIndex: 0 }));
              }}
            />
          </div>
                ...
                {table.getRowModel().rows.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={11} className="text-center py-6 text-sm text-gray-500">
                      No sale rows found for reverse criteria.
                    </TableCell>
                  </TableRow>
                ) : (
              ...
              <div className="mb-4 p-3 bg-gray-50 rounded">
                ...
                <div className="grid grid-cols-2 gap-2 mt-2">
                  ...
                  <div>
                    ...
                    <p className="font-semibold">{selectedGroup.records.length}</p>
                  </div>
                </div>
              </div>
                  ...
                  <TableBody>
                    {selectedGroup.records.map((record, idx) => (
                      <TableRow key={idx} className="hover:bg-gray-50">
                        ...
                        <TableCell className="p-2 border text-center text-xs">{record.quantity}</TableCell>
                        <TableCell className="p-2 border text-center text-xs">
                        ...
                        <TableCell className="p-2 border text-center text-xs">{record.urn_number || "-"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
              ...
              <div className="mt-4 p-3 bg-gray-50 rounded">
                <div className="grid grid-cols-4 gap-2">
                  <div>
                    ...
                    <p className="font-semibold">{selectedGroup.totalQuantity}</p>
                  </div>
                  <div>
...
export default ReverseSaleInvoicePage;
