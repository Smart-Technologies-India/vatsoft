...
const DocumentWiseDetails = () => {
  ...
  const [workflowGroup, setWorkflowGroup] = useState<GroupedDailySale | null>(
    null,
  );
  const [isWorkflowModalOpen, setIsWorkflowModalOpen] = useState(false);
  const [approvedInvoices, setApprovedInvoices] = useState<string[]>([]);
  const [shippedInvoices, setShippedInvoices] = useState<string[]>([]);
  ...
  const getInvoiceKey = (group: GroupedDailySale): string => {
    return `${group.invoice_number}-${group.seller_tin_number.tin_number}-${new Date(group.invoice_date).toISOString()}`;
  };

  const openWorkflowModal = (group: GroupedDailySale) => {
    setWorkflowGroup(group);
    setShipmentDetails({ vehicleNo: "", shipmentTime: "" });
    setIsWorkflowModalOpen(true);
  };

  const isApproved = (group: GroupedDailySale | null): boolean => {
    if (!group) return false;
    return approvedInvoices.includes(getInvoiceKey(group));
  };

  const isShipped = (group: GroupedDailySale | null): boolean => {
    if (!group) return false;
    return shippedInvoices.includes(getInvoiceKey(group));
  };

  const workflowSteps = useMemo(() => {
    const approved = isApproved(workflowGroup);
    const shipped = isShipped(workflowGroup);

    return [
      { label: "Accept invoice", done: Boolean(workflowGroup) },
      { label: "Tax Paid", done: Boolean(workflowGroup) },
      { label: "Approve by refinery", done: approved },
      { label: "Shipped", done: shipped },
    ];
  }, [workflowGroup, approvedInvoices, shippedInvoices]);
  ...
  const handleApproveAndRelease = () => {
    ...
    setApprovedInvoices((prev) => (prev.includes(key) ? prev : [...prev, key]));
    setShippedInvoices((prev) => (prev.includes(key) ? prev : [...prev, key]));
    toast.success("Approved and marked as shipped.");
  };
  ...
  return (
    <>
      ...
      <Modal
        title="Invoice Shipment Workflow"
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <Input
                  ...
                  disabled={isShipped(workflowGroup)}
                />
              </div>
              <div>
            ...
            <div className="flex items-center justify-between gap-3">
              <Button
                ...
                disabled={isShipped(workflowGroup)}
              >
                {isShipped(workflowGroup) ? "Shipped" : "Approve and Release"}
              </Button>
              {isShipped(workflowGroup) && (
                      ...
                      >
                        ...
                        <TableCell className="p-2 text-center text-xs">
                          <button
                            ...
                            className="text-sm bg-blue-500 hover:bg-blue-600 text-white py-1 px-4 rounded"
                          >
                            View
                          </button>
