...
const AddPurchase = () => {
  ...
  const [purchaseBox, setPurchaseBox] = useState<boolean>(false);
  ...
  const submit = async () => {
    ...
    const entries = purchases.map((purchase) => ({
      ...
      commodityid: purchase.item.id,
      quantity: purchase.quantity,
      seller_tin_id: 1, // Default TIN ID for unregistered/cash purchases
      invoice_number: purchase.invoice_number,
      invoice_date: new Date(purchase.invoice_date),
      tax_percent: purchase.item.taxable_at || "0",
      amount: purchase.net_amount.toString(),
      vatamount: ((purchase.net_amount * parseFloat(purchase.item.taxable_at || "0")) / 100).toFixed(2),
  ...
  return (
    <>
      <Drawer
        ...
        open={purchaseBox}
      >
        <p className="text-lg text-left mb-4">Add Multiple Purchase Invoices</p>
        <MultiplePurchaseForm
          setAddBox={setPurchaseBox}
      ...
      <main className="w-full p-4">
        <div className="bg-white px-4 py-2 mt-2">
          <p className="text-2xl font-semibold mb-4">Add Purchase Invoices</p>
              ...
              {purchases.length != 0 ? (
                <>
                  <Table className="border mt-2">
                    ...
                    <TableBody>
                      {purchases.map((val: PurchaseData, index: number) => (
                        <TableRow key={index}>
                ...
                <Alert
                  ...
                  type="error"
                  showIcon
                  description="There are no purchase invoices. Click 'Add Purchase Invoices' to add items."
                />
              )}
            </>
          )}
      ...
      <Modal title="Confirmation" open={open} footer={null} closeIcon={false}>
        <div>
          <p>
            You are about to submit {purchases.length} purchase invoice(s) for{" "}
        ...
        <div className="mt-4 flex items-start gap-2 p-3 bg-slate-50 rounded-lg border border-slate-200">
          ...
          >
            I confirm that all purchase invoice information provided is accurate and
            complete.
          </label>
        </div>
      ...
      {/* Bulk Upload Modal */}
      <Modal
        title="Bulk Upload Purchase Data"
...
// Multiple Purchase Form Component
interface MultiplePurchaseFormProps {
...
const MultiplePurchaseForm = (props: MultiplePurchaseFormProps) => {
  const [commodityMaster, setCommodityMaster] = useState<commodity_master[]>(
    []
  );
  ...
  useEffect(() => {
    const init = async () => {
      ...
      if (commodity_response.status && commodity_response.data) {
        ...
        } else if (props.dvatdata?.commodity == "OIDC") {
          products = products.filter((val) => val.product_type == "LIQUOR");
        } else {
          products = products.filter((val) => val.product_type == props.dvatdata?.commodity);
        }
  ...
  const handleSubmit = () => {
    ...
    if (newPurchaseItems.length > 0) {
      props.setPurchases([...props.purchases, ...newPurchaseItems]);
      toast.success(`${newPurchaseItems.length} purchase invoice(s) added`);
      ...
      <div className="mb-4 p-3 bg-blue-50 rounded-lg border border-blue-200">
        ...
        {selectedMonth && invoiceStart && (
          <p className="text-xs text-gray-600 mt-2">
            Format: {invoiceStart}/{selectedMonth.format("MMYYYY")}
          </p>
        )}
      </div>
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <input
                      ...
                      disabled={!selectedMonth || !invoiceStart}
                    />
      ...
      <div className="mt-3 pt-3 border-t">
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={
              selectedProducts.size === 0 || !selectedMonth || !invoiceStart
            }
          >
