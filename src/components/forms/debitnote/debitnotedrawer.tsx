...
import CreateDebitNote from "@/action/return/createdebitnote";
import { FormProvider, useForm, useFormContext } from "react-hook-form";
import { DebitNoteForm, DebitNoteSchema } from "@/schema/debitnote";
import { valibotResolver } from "@hookform/resolvers/valibot";
import { TaxtInput } from "../inputfields/textinput";
import { DateSelect } from "../inputfields/dateselect";

type DebitNoteDrawerProps = {
...
export const DebitNoteDrawer = (props: DebitNoteDrawerProps) => {
  const methods = useForm<DebitNoteForm>({
    resolver: valibotResolver(DebitNoteSchema),
    defaultValues: {
  ...
  return (
    <FormProvider {...methods}>
      <DebitNoteDrawerForm {...props} />
    </FormProvider>
  );
};

const DebitNoteDrawerForm = (props: DebitNoteDrawerProps) => {
  ...
  const onSubmit = async (data: DebitNoteForm) => {
    try {
      ...
      toast.error("Failed to submit debit note.");
    }
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-2 justify-end pt-2 border-t">
        ...
        >
          {isSubmitting ? "Submitting..." : "Submit Debit Note"}
        </button>
      </div>
    </form>
  );
