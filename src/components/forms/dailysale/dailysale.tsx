...
const DailySale = (props: DailySaleProviderProps) => {
  ...
  useEffect(() => {
    ...
    init();
  }, [recipient_vat_no, againstType, isComp]);
  ...
  useEffect(() => {
    ...
    init();
  }, [description_of_goods, againstType, isComp]);
  ...
  const onSubmit = async (data: DailySaleForm) => {
    ...
    if (
      ...
      if (davtdata?.commodity == "OIDC") {
        ...
        if (
          isLiquore &&
          commoditymaster.pack_size &&
          Number(commoditymaster.pack_size) > 0 &&
      ...
      >
        ...
        <div className="flex gap-1 items-center">
          ...
          <div className="mt-2 bg-gray-100 rounded p-2  flex-1">
            ...
            <p className="text-sm font-semibold">
              {formatAmount(Number(taxableValue) || 0)}
              {/* {formatAmount(
                (((Number(quantity) || 0) * (Number(amount_unit) || 0)) /
                  (100 +
                    parseFloat(
