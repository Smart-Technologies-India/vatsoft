...
const AddCreditDebitNote = ({ 
  ...
  const [noteMode, setNoteMode] = useState<"credit" | "debit" | "goods-return">(mode);
  ...
  useEffect(() => {
    const loadDrawerData = async () => {
      ...
      if (commodityResponse.status && commodityResponse.data) {
        ...
        const filteredCommodities = commodityResponse.data.filter(
          (commodity) => [1, 2, 748, 749].includes(commodity.id)
        );
  ...
  const handleCreateNote = async (data: CreditDebitNoteFormValues) => {
    ...
    try {
      const response = await CreateCreditDebitNote({
      ...
      if (response.status) {
        toast.success(response.message || "Credit/Debit note created successfully.");
        handleClose();
        onCreated?.();
      } else {
