...
const ConvertDvat30A = async (
  ...
  try {
    ...
    for (let start = 0; start < totalRows; start += chunkSize) {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (updateResponse.count !== chunkRows.length) {
          throw new Error("Unable to update one or more purchase rows.");
        }
      });
    }
    ...
    return createResponse({
      message: `Convert to DVAT 30/30 A Completed. Processed ${totalRows} purchase row(s) in ${totalChunks} batch(es) of ${chunkSize}.`,
      functionname,
