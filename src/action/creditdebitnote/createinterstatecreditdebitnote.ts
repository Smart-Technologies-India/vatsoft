...
interface CreateInterstateCreditDebitNotePayload {
...
const CreateInterstateCreditDebitNote = async (
  payload: CreateInterstateCreditDebitNotePayload,
): Promise<ApiResponseType<CreditDebitNoteWithRelations | null>> => {
  const functionname: string = CreateInterstateCreditDebitNote.name;
  ...
  try {
    ...
    // Validate TIN does not start with 25 or 26 (interstate only)
    if (
      tinNumber.tin_number.startsWith("25") ||
      tinNumber.tin_number.startsWith("26")
    ) {
      return createResponse({
        message:
          "Interstate credit/debit notes can only be created with inter-state TIN numbers.",
        functionname,
      });
    }
    ...
    // Create only SALE entry (DVAT_31) for interstate credit/debit notes
    // No purchase entry created
    await prisma.returns_entry.create({
      data: {
        ...
        seller_tin_numberId: payload.seller_tin_number_id,
        category_of_entry: payload.is_credit
    ...
    return createResponse({
      message: "Interstate Credit/Debit note created successfully.",
      functionname,
      data: note,
    });
  } catch (error) {
...
export default CreateInterstateCreditDebitNote;
